...
RSpec.describe EntityCategory, type: :model do
  ...
  let(:category) { GroupCategory.create!(user:, name: 'Albert', icon: GroupCategory::ICONS.keys.first) }
  let(:entity_category) do
    EntityCategory.create!(user:, name: 'Albert', amount: 500, group_category: category)
  end
