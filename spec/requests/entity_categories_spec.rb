...
RSpec.describe 'EntityCategories', type: :request do
  ...
  let(:user) { User.create!(full_name: 'Albert Antwi', email: 'albertkantwi@test.com', password: 'password') }
  let(:category) { GroupCategory.create!(name: 'Journey', icon: GroupCategory::ICONS.keys.first, user:) }

  let(:valid_params) { { name: 'Noah', amount: 5, group_category_id: category.id } }
